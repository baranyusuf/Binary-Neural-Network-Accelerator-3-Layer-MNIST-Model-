...
class BNN_MNIST:
    ...
    def matmul_vs_xnormatmul(self):
        ...
        print(
            "Matmul result: {}, XNOR result: {}".format(
                matmul_result, 2 * xnormatmul_sum - len(a)
            )
        )
    ...
    def concat4(self, li, point):
        ...
        :return:
        """
        result = np.array([self.quantize_scale(li[point])], dtype=np.uint32)[0].astype(
            np.uint32
        )
    ...
    def preprocessModel(self, X, y):
        ...
        np.save(
            "dataset/mnist-bit_sample{}.npy".format(sample),
            numpydict,
            allow_pickle=True,
        )
    ...
    def create_input(self, num_of_samples):
        ...
        np.save(
            "dataset/mnist-bit_sample{}.npy".format(num_of_samples),
            numpydict,
            allow_pickle=True,
        )
    ...
    def hlscode(self):
        ...
        def to_c_1d_array(lst):
            return "{" + ", ".join(map(str, lst)) + "}"

        mnist_input = np.load(
            "dataset/mnist-bit_sample10.npy", allow_pickle=True
        ).item()
        X_input = mnist_input.get("X")
        y_input = mnist_input.get("y")

        mnist_output = np.load(
            "dataset/mnist_test_data_original.npy", allow_pickle=True
        ).item()
        X = mnist_output.get("data")
        y = mnist_output.get("label")
        ...
        for x in X[:10]:
            x = x.reshape(1, -1)
            A, layer1_output, layer2_output, layer3_output = bnn.feed_forward_quantized(
                x
            )
            outputs.append(A)
            layer1_outputs.append(layer1_output)
            layer2_outputs.append(layer2_output)
            layer3_outputs.append(layer3_output)

        clean_outputs = np.vstack(outputs).astype(int).tolist()
        clean_layer1 = np.vstack(layer1_outputs).astype(int).tolist()
        clean_layer2 = np.vstack(layer2_outputs).astype(int).tolist()
        clean_layer3 = np.vstack(layer3_outputs).astype(int).tolist()

        def load_layer(filename):
            with open(filename, "r") as f:
                return [int(x) for x in f.readline().split(",") if x.strip() != ""]

        layer1 = load_layer("weights/layer1_c.txt")
        layer2 = load_layer("weights/layer2_c.txt")
        layer3 = load_layer("weights/layer3_c.txt")

        with open("../golden.h", "w") as fh:
            fh.write("#ifndef __BNN_GOLDEN_HEADER_\n")
            fh.write("#define __BNN_GOLDEN_HEADER_\n")
            fh.write("#include <cstdint>\n\n")

            fh.write(
                f"extern const uint32_t INPUT_BITS[10][25] = {to_c_2d_array(X_input)};\n"
            )
            fh.write(
                f"extern const int OUTPUT[10][10] = {to_c_2d_array(clean_outputs)};\n"
            )
            fh.write(
                f"extern const int LAYER1_OUTPUT[10][128] = {to_c_2d_array(clean_layer1)};\n"
            )
            fh.write(
                f"extern const int LAYER2_OUTPUT[10][64] = {to_c_2d_array(clean_layer2)};\n"
            )
            fh.write(
                f"extern const int LAYER3_OUTPUT[10][10] = {to_c_2d_array(clean_layer3)};\n"
            )
            fh.write(f"extern const uint32_t LAYER1[3200] = {to_c_1d_array(layer1)};\n")
            fh.write(f"extern const uint32_t LAYER2[256] = {to_c_1d_array(layer2)};\n")
            fh.write(f"extern const uint32_t LAYER3[20] = {to_c_1d_array(layer3)};\n")
    ...
    def feed_forward_quantized(self, input):
        ...
        return A, layer1_activations, layer2_activations, final_output
    ...
    def visualize(self, data, true_label, predicated_label):
        ...
        plt.title(
            "true label: {}, predicted label :{}".format(true_label, predicated_label)
        )
